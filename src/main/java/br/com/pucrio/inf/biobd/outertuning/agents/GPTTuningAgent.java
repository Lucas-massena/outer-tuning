...
public class GPTTuningAgent {

    public static final String HEURISTIC_NAME = "GPTTuning";
    private static final String GPT_URL = "https://api.openai.com/v1/chat/completions";

    private final Configuration config;

    public GPTTuningAgent(Configuration config) {
        this.config = config;
    }
    ...
    public static Heuristic getHeuristic() {
        ...
        h.setVersion("1");
        h.setStrategy("GPT Tuning Suggestion");
        h.setAuthor("OpenAI/Lucas Ferraz Massena");
        return h;
    }
    ...
    public List<ActionSF> analyze(CopyOnWriteArrayList<SQL> sqlList, Schema schema) {
        ...
        if (apiKey == null || apiKey.trim().isEmpty()) {
            System.out.println("[GPTTuning] API key not configured. " +
                "Set OPENAI_API_KEY env var or gptApiKey in properties.");
            return actions;
        }
        ...
        try {
            ...
            String response = callGPT(prompt, apiKey);
            if (response != null) {
                actions = parseResponse(response, sqlList);
                System.out.println("[GPTTuning] Received " + actions.size() + " suggestion(s).");
            }
        } catch (Exception e) {
            System.out.println("[GPTTuning] Error: " + e.getMessage());
        }
        return actions;
    }
    ...
    private String getApiKey() {
        ...
        if (key == null || key.isEmpty()) {
            key = config.getProperty("gptApiKey");
        }
        return key;
    }
    ...
    private String callGPT(String prompt, String apiKey) throws Exception {
        ...
        if (code != 200) {
            System.out.println("[GPTTuning] API error " + code + ": " + result);
            return null;
        }
    ...
    private List<ActionSF> parseResponse(String response, CopyOnWriteArrayList<SQL> sqls) {
        ...
        try {
            ...
            String text = resp.getAsJsonArray("choices").get(0)
            ...
            for (int i = 0; i < suggestions.size(); i++) {
                ...
                if ("MATERIALIZED_VIEW".equals(type)) {
                    System.out.println("[GPTTuning] Skipping MATERIALIZED_VIEW suggestion for SQL#" + sqlId);
                    continue;
                }

                ActionSF action = new ActionSF();
                action.setId("GPT_" + sqlId + "_" + i);
                action.setName("GPTSuggestion_SQL" + sqlId + "_INDEX_" + i);
                ...
                if (linkedSql != null && !cmd.isEmpty()
                    ...
                    if (costWithIndex >= 0) {
                        ...
                        System.out.println("[GPTTuning] SQL#" + sqlId
            ...
            System.out.println("[GPTTuning] Parse error: " + e.getMessage());
        }
        return actions;
    }
    ...
    private float[] estimateCostWithMysqlIndex(String indexCommand, String querySql) {
        ...
        try {
            ...
            System.out.println("[GPTTuning] estimateCostWithMysqlIndex error: " + e.getMessage());
    ...
    private float[] estimateCostWithHypoPG(String indexCommand, String querySql) {
        ...
        try {
            ...
            System.out.println("[GPTTuning] estimateCostWithHypoPG error: "
    ...
    private float estimateIndexCreationCost(Statement stmt, String tableName,
        ...
        try (ResultSet rs = stmt.executeQuery(sql)) {
            ...
            System.out.println("[GPTTuning] estimateIndexCreationCost error: " + e.getMessage());
            return 0f;
        }

        System.out.println("[GPTTuning] creationCost stats: table=" + simple
        ...
        if (reltuples < 1) {
            ...
            System.out.println("[GPTTuning] reltuples from pg_stat_user_tables: " + reltuples);
        }
        if (reltuples < 1) {
            try (ResultSet cnt = stmt.executeQuery("SELECT COUNT(*) FROM \"" + simple + "\"")) {
                if (cnt.next()) reltuples = cnt.getLong(1);
            } catch (Exception e) {
                System.out.println("[GPTTuning] COUNT fallback failed for " + simple + ": " + e.getMessage());
            }
            System.out.println("[GPTTuning] reltuples from COUNT(*): " + reltuples);
        }
