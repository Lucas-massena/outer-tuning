...
public class ClaudeTuningAgent {

    public static final String HEURISTIC_NAME = "ClaudeTuning";
    private static final String CLAUDE_URL = "https://api.anthropic.com/v1/messages";

    private final Configuration config;

    public ClaudeTuningAgent(Configuration config) {
        this.config = config;
    }
    ...
    public static Heuristic getHeuristic() {
        ...
        h.setVersion("1");
        h.setStrategy("Claude Tuning Suggestion");
        h.setAuthor("Anthropic/Lucas Ferraz Massena");
        return h;
    }
    ...
    public List<ActionSF> analyze(CopyOnWriteArrayList<SQL> sqlList, Schema schema) {
        ...
        if (apiKey == null || apiKey.trim().isEmpty()) {
            System.out.println("[ClaudeTuning] API key not configured. " +
                "Set ANTHROPIC_API_KEY env var or claudeApiKey in outertuning.properties.");
            return actions;
        }
        ...
        try {
            ...
            String response = callClaude(prompt, apiKey);
            if (response != null) {
                actions = parseResponse(response, sqlList);
                System.out.println("[ClaudeTuning] Received " + actions.size() + " suggestion(s).");
            }
        } catch (Exception e) {
            System.out.println("[ClaudeTuning] Error: " + e.getMessage());
        }
        return actions;
    }
    ...
    private String getApiKey() {
        ...
        if (key == null || key.isEmpty()) {
            key = config.getProperty("claudeApiKey");
        }
        return key;
    }
    ...
    private String callClaude(String prompt, String apiKey) throws Exception {
        ...
        if (code != 200) {
            System.out.println("[ClaudeTuning] API error " + code + ": " + result);
            return null;
        }
    ...
    private List<ActionSF> parseResponse(String response, CopyOnWriteArrayList<SQL> sqls) {
        ...
        try {
            ...
            String text = resp.getAsJsonArray("content").get(0)
            ...
            for (int i = 0; i < suggestions.size(); i++) {
                ...
                if ("MATERIALIZED_VIEW".equals(type)) {
                    System.out.println("[ClaudeTuning] Skipping MATERIALIZED_VIEW suggestion for SQL#" + sqlId);
                    continue;
                }

                ActionSF action = new ActionSF();
                action.setId("CLAUDE_" + sqlId + "_" + i);
                action.setName("ClaudeSuggestion_SQL" + sqlId + "_INDEX_" + i);
                ...
                if (linkedSql != null && !cmd.isEmpty()
                    ...
                    if (costWithIndex >= 0) {
                        ...
                        System.out.println("[ClaudeTuning] SQL#" + sqlId
            ...
            System.out.println("[ClaudeTuning] Parse error: " + e.getMessage());
        }
        return actions;
    }
    ...
    private float[] estimateCostWithMysqlIndex(String indexCommand, String querySql) {
        ...
        try {
            ...
            try { stmt.execute("DROP INDEX " + indexName + " ON " + tableName); } catch (Exception ignored) {}
            ...
            try {
                ...
                if (rs != null && rs.next()) {
                    ...
                    queryCost = Float.parseFloat(explain.getAsJsonObject("query_block")
                            .getAsJsonObject("cost_info").get("query_cost").getAsString());
                    rs.close();
                }
            } finally {
                try { stmt.execute("DROP INDEX " + indexName + " ON " + tableName); } catch (Exception ignored) {}
            ...
            System.out.println("[ClaudeTuning] estimateCostWithMysqlIndex error: " + e.getMessage());
    ...
    private float[] estimateCostWithHypoPG(String indexCommand, String querySql) {
        ...
        try {
            ...
            System.out.println("[ClaudeTuning] estimateCostWithHypoPG error: "
    ...
    private float estimateIndexCreationCost(Statement stmt, String tableName,
        ...
        try (ResultSet rs = stmt.executeQuery(sql)) {
            ...
            System.out.println("[ClaudeTuning] estimateIndexCreationCost error: " + e.getMessage());
            return 0f;
        }

        System.out.println("[ClaudeTuning] creationCost stats: table=" + simple
        ...
        if (reltuples < 1) {
            ...
            System.out.println("[ClaudeTuning] reltuples from pg_stat_user_tables: " + reltuples);
        }
        if (reltuples < 1) {
            try (ResultSet cnt = stmt.executeQuery("SELECT COUNT(*) FROM \"" + simple + "\"")) {
                if (cnt.next()) reltuples = cnt.getLong(1);
            } catch (Exception e) {
                System.out.println("[ClaudeTuning] COUNT fallback failed for " + simple + ": " + e.getMessage());
            }
            System.out.println("[ClaudeTuning] reltuples from COUNT(*): " + reltuples);
        }
