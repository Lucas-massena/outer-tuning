...
public class PlanPostgreSQL extends Plan {
    ...
    public long getCost() {
        if (!this.getPlan().isEmpty()) {
            try {
                // EXPLAIN format: "(cost=X..Y rows=...)" — total cost is after ".."
                int dotDot = this.getPlan().indexOf("..");
                if (dotDot < 0) return 0;
                int start = dotDot + 2;
                int end = this.getPlan().substring(start).indexOf(" ") + start;
                return Math.round(Float.parseFloat(this.getPlan().substring(start, end)));
    ...
    public float getDuration() {
        if (!this.getPlan().isEmpty()) {
            try {
                // PostgreSQL 14+: "Execution Time:" / older: "Execution time:"
                String planLower = this.getPlan().toLowerCase();
                int execIdx = planLower.indexOf("execution time:");
                if (execIdx < 0) return 0;
                int ini = execIdx + 15;
                int end = planLower.indexOf("ms", ini);
                if (end < 0) return 0;
                float exec = Float.parseFloat(this.getPlan().substring(ini, end).trim());

                int planIdx = planLower.indexOf("planning time:");
                if (planIdx >= 0) {
                    int pIni = planIdx + 14;
                    int pEnd = planLower.indexOf("ms", pIni);
                    if (pEnd >= 0) {
                        exec += Float.parseFloat(this.getPlan().substring(pIni, pEnd).trim());
                    }
                }
                return exec;
            } catch (NumberFormatException e) {
