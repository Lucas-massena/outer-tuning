...
public class ServletAgents extends ServletBase {
    ...
    public void aiComparison() {
        this.checkObjectsMemory();
        if (this.controller.isRunning()) {
            List<CoordenatorAgent.AIComparisonRow> rows = this.controller.getAIComparisonRows();
            request.setAttribute("comparisonRows", rows);
            request.setAttribute("comparisonChartData", this.controller.getAIComparisonChartData(rows));
        }
    }
    ...
    public void workload() {
        ...
        if (!this.controller.isAnyHeuristicSelected()) {
